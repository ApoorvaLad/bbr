...
TcpSocketBase::Send (Ptr<Packet> p, uint32_t flags)
{
  ...
  //std::cerr<<m_congestionControl->const<<std::endl;
 // std::cerr<<"Send packet"<<std::endl;
 // m_congestionControl->Send(m_tcb);
  ...
  if (m_state == ESTABLISHED || m_state == SYN_SENT || m_state == CLOSE_WAIT)
    {
      ...
      // std::cerr<<"Sending Time for packet "<<p->GetUid()<<" at "<<Simulator::Now().GetSeconds()<<std::endl;
...
TcpSocketBase::ReceivedAck (Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
  NS_LOG_FUNCTION (this << tcpHeader);
     ...
     // std::cerr<<previousPacket<<std::endl;
     // std::cerr<<"Received "<<tcpHeader.GetSequenceNumber()<<" "<<Simulator::Now().GetSeconds()<<std::endl;


      if((previousPacket == 0) || (previousPacket != id)){
       // m_rtt = receiveTime - sendTime;
       // std::cerr<<"Packet Id" <<tcpHeader.GetSequenceNumber()<<" "<<Simulator::Now().GetSeconds()<<std::endl;
      //  NS_LOG_INFO (Simulator::Now().GetSeconds()<<" "<<m_rtt.GetSeconds() << " ");
      }

      previousPacket = id;

  //std::cerr<<"Receiveing Time for packet "<< packet->GetUid()<<" at "<<Simulator::Now().GetSeconds()<<std::endl;
          ...
          if (ackNumber < m_recover)
            {
              ...
              std::cerr<<"Congestion window is not called"<<std::endl;
...
TcpSocketBase::SendDataPacket (SequenceNumber32 seq, uint32_t maxSize, bool withAck)
{
  ...
  //std::cerr<<"Sent "<<seq<<" "<<Simulator::Now().GetSeconds()<<std::endl;
...
TcpSocketBase::ReceivedData (Ptr<Packet> p, const TcpHeader& tcpHeader)
{
  ...
  //std::cerr<<p->GetUid()<<std::endl;

//  std::cerr<<tcpHeader.GetSequenceNumber()<<std::endl;
  m_receiveTime = Simulator::Now();
  m_congestionControl->Recv(tcpHeader,m_receiveTime,p->GetSize());
  NS_LOG_FUNCTION (this << tcpHeader);
...
TcpSocketBase::EstimateRtt (const TcpHeader& tcpHeader)
{
  ...
  if (!m.IsZero ())
    {
      m_rtt->Measurement (m); 
      //std::cerr<<"Rtt is "<<m_rtt<<std::endl;               // Log the measurement
      // RFC 6298, clause 2.4
      m_rto = Max (m_rtt->GetEstimate () + Max (m_clockGranularity, m_rtt->GetVariation () * 4), m_minRto);
...
TcpSocketBase::NewAck (SequenceNumber32 const& ack, bool resetRTO)
{
  ...
  if (m_state != SYN_RCVD && resetRTO)
    { // Set RTO unless the ACK is received in SYN_RCVD state
      ...
      // RFC 6298, clause 2.4
      m_rto = Max (m_rtt->GetEstimate () + Max (m_clockGranularity, m_rtt->GetVariation () * 4), m_minRto);
     // std::cerr<<"Received "<< ack<<" at "<< m_rtt->GetEstimate().GetSeconds()<<std::endl;
      NS_LOG_LOGIC (this << " Schedule ReTxTimeout at time " <<
                    Simulator::Now ().GetSeconds () << " to expire at time " <<
                    (Simulator::Now () + m_rto.Get ()).GetSeconds ());
      m_retxEvent = Simulator::Schedule (m_rto, &TcpSocketBase::ReTxTimeout, this);
       //std::cerr<<ack<<" "<<Simulator::Now().GetSeconds()<< " "<<(m_receiveTime - m_sendTime).GetSeconds()<<std::endl;
    }
...
TcpSocketBase::SetCongestionControlAlgorithm (Ptr<TcpCongestionOps> algo)
{
 ...
 // std::cerr<<"It enetered HEre"<<std::endl;
